...
A common initialization mistake involves loading too many skills simultaneously. Each skill adds tokens to every subsequent request, increases initialization time, and can create conflicting instructions. The best practice is to load only skills directly relevant to your current work.
...
Use directory-specific CLAUDE.md files for monorepos or large codebases. Place a CLAUDE.md in each package or module directory that describes only that component:
...
Never include sensitive values in CLAUDE.md or any configuration file that might be committed to version control. Instead, establish environment variable patterns during initialization.
