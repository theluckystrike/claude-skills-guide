...
A God Class — a class that has accumulated too many responsibilities — is one of the most common and damaging patterns in long-lived codebases. Here is how the workflow above plays out against a real example.

Say you have a `UserManager` class with 800 lines handling authentication, profile updates, notification preferences, billing, and session management all in one place.

**Step 1: Assess the scope.** Ask Claude Code to map the responsibilities:
