...
The script above catches known signatures, but sophisticated miners obfuscate their filenames. A more robust approach monitors CPU behavior directly using the Performance API:
...
class MiningDetector {
  ...
  _sample() {
    ...
    const utilization = Math.max(0, 100 - (count / 50000) * 100);
    this.samples.push({ time: Date.now(), cpu: utilization });
