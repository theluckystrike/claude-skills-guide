...
4. **Ignores n8n's expression syntax.** Claude hardcodes values in nodes. n8n uses `{{ $json.fieldName }}` expressions to reference data from previous nodes in the workflow chain.
...
1. **Expression vs static value confusion.** Claude sets node parameters as static strings when they should be expressions referencing previous nodes. In n8n, use the `=` toggle on fields to switch to expression mode and reference `{{ $json.data }}`.
