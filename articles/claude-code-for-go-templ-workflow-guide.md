---
layout: default
title: "Claude Code for Go Templ — Workflow Guide"
description: "Build type-safe Go HTML templates with Templ and Claude Code. Tested setup with copy-paste CLAUDE.md config."
date: 2026-04-18
...
1. **Uses Go's `html/template` package.** Claude writes `{{ .Title }}` Go template syntax with `template.ParseFiles()`. Templ uses a JSX-like syntax in `.templ` files that compiles to type-safe Go functions.
...
4. **Ignores component composition.** Claude creates monolithic templates with `{{ template "header" . }}` includes. Templ supports proper component composition — `@Header()` calls inside `templ Page()` for nested, reusable components.
...
2. **Using Go template syntax in .templ files.** Claude writes `{{ .Title }}` inside `.templ` files. Templ uses `{ title }` without double braces — the syntax is different from Go's `html/template`.
