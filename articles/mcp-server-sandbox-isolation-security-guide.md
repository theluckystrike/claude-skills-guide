...
This approach is more robust than application-level network config because it enforces the policy in the kernel. Even if the MCP server's own code is modified to bypass its configuration, the kernel rules still apply.
